"""React-экран «Период» копирует слепок и сам его не считает."""
...
def _render(view: dict) -> str:
    proc = subprocess.run(
        ["node", "frontend/render.mjs", "period"],
...
def _page(case: str, tmp_path: Path, edits: bool = False) -> str:
    if not edits:
        ...
        path = result.path
    return _render(team_view(load_snapshot(path)))
...
def test_edit_form_names_the_server_fields():
    page = _render(
        {
            "teamId": "card",
            "coverage": {"timing": "full", "membership": "complete"},
    ...
    assert 'name="epicId"' in page
    assert 'name="release"' in page
    assert 'name="text"' in page
    assert 'name="author"' in page
